...
public:
	long long minArea(vector <int> x, vector <int> y) {
		int N = x.size();
		vector<long long int> subsets;
		subsets.push_back(0);
		subsets.push_back(0);
		for(int i = 1 ; i<=N-2 ; i++)
		subsets.push_back(1);
		long long int maxArea = 5000000000000000000;
		do{
			long long int minx = 1000000002;
			long long int maxx = -1000000002;
			long long int miny = 1000000002;
			long long int maxy = -1000000002;
			...
			miny--;
			maxy++;
			long long int side = maxx-minx > maxy-miny ? maxx-minx : maxy-miny;
			long long int local = (side)*(side);
			if(local<maxArea)
			maxArea=local;			
		}
		while(std::next_permutation(subsets.begin(),subsets.end()));
		return maxArea;
	}
};
