...
public:
	int spacesUsed(vector <string> events) {
		...
		for(int i = 0 ; i < events.size() ; i++){
			string str = events[i].substr(0,events[i].size()-8);
			int j;
			for(j = 0 ; j < dist.size() ; j++){
				if(dist[j].compare(str)==0)
				break;
			}
			if(j==dist.size())
			{
		...
		vector<bool> arrived(dist.size(),false);
		int current=0;
		int ans = 0;
		for(int i = 0 ; i < events.size() ; i++){
			string left = events[i].substr(0,events[i].size()-8);
			if(events[i][left.size()+1] =='a'){
				current++;
				for(int j = 0 ; j < dist.size() ; j++){
					if(dist[j].compare(left)==0){
						arrived[j]=true;
						break;	
					}
				}
