...
public:
	int maxProfit(vector <int> cityValues, vector <string> roads) {
			...
			bool visited[51][1023];
			int max=-1,x,y,mask,newMask;
			queue<int> que;
			
		memset(visited,0,sizeof(visited));
		que.push(0); que.push(cityValues[0]);
		...
		while(!que.empty()){
			...
			for(int i = 0 ; i<roads[0].size() ;i++){
				y=i;
				newMask = mask ^ cityValues[y];
				if(roads[x][y]=='N')
				continue;
				if(!visited[y][newMask]){
					que.push(y); que.push(newMask);
					visited[y][newMask]=true;
				}
			}
		}
		return max;	
