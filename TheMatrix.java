...
public class TheMatrix
{
	public int MaxArea(String[] board)
	{
		...
		for(int i = 0 ; i < col ; i++){
			...
			for(int length = i ; length<col ; length++){
					String temps[] = new String[row];
					for(int k = 0 ; k<row ;k++){
						if(length-i+1==1){
							try{
							temps[k]=new String(board[k].substring(i,length+1));
							}
							catch(Exception E){
							...
							int j;
							for(j = i ;j<length ; j++){
								if(board[k].charAt(j)==board[k].charAt(j+1))
								break;
							}
							if(j==length)
							temps[k]=new String(board[k].substring(i,length+1));
					...
					for(int k = start+1 ; k<row ;k++){
						if(temps[k]!=null){
							if(start==-1)
							{ start = k; res = length-i+1; continue;}
							int m = 0;
							for(;m<length-i+1 ;m++){
								if(temps[k].charAt(m)==temps[start].charAt(m))
								break;	
							}
							if(m==length-i+1){
								res=res+length-i+1;
