...
public:
	...
	int decompose(string sentence, vector <string> validWords) {
		vector<int> dp(55,0);
		vector<string> ascValidWords=validWords;
		for(int i = 0 ; i<validWords.size() ;i++){
			sort(ascValidWords[i].begin(),ascValidWords[i].end());
		}
		for(int i = sentence.size()-1 ;i>=0 ;i--){
			dp[i]=inf;
			for(int j = 0 ; j<validWords.size();j++){
				int n = validWords[j].size();
				if(n<=sentence.size()-i){
					string str=	sentence.substr(i,n);
					sort(str.begin(),str.end());
					if(str.compare(ascValidWords[j])==0){
						int local=0;
						for(int k = i ;k<i+n ;k++)
						if(sentence[k]!=validWords[j][k-i])
						local++;
						dp[i]=min(dp[i],local+dp[i+n]);
					}
				}	
			}
		}
