...
public:
	int belowAvg(vector <int> math, vector <int> verbal) {
		vector<int> composite(math.size());
		double totalMath=0,totalVerbal=0;
		for(int i = 0 ; i < math.size() ; i++){
			totalMath+=math[i];
			totalVerbal+=verbal[i];
			composite[i]=math[i]+verbal[i];
		}
		totalMath/=math.size();
		totalVerbal/=math.size();
		double totalScore = totalMath+totalVerbal;
		int count=0;
		for(int i = 0 ; i < math.size() ; i++){
			if(totalScore>composite[i])
			count++;
		}
		return count;
	}
